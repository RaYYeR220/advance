...
import styles from "../auctions/AuctionEmptyStates.module.css";
...
export function LoanNotFound({ loanId }: { loanId: string }) {
  return (
    <Shell eyebrow="Not found" title={`No loan #${loanId} has opened`}>
      <p className={styles.body}>The hub has never opened a loan with this id.</p>
      <p className={styles.actions}>
        <TextLink href="/auctions">Back to auctions</TextLink>
      </p>
    </Shell>
...
export function LoanError({ loanId }: { loanId: string }) {
  return (
    <Shell eyebrow="Not available" title="Can't read this loan right now">
      <p className={styles.body}>
        The chain/activity read for loan <code>{loanId}</code> failed partway through — this is usually transient.
      </p>
      <p className={styles.actions}>
        <TextLink href={`/loans/${loanId}`}>Try again</TextLink> · <TextLink href="/auctions">Back to auctions</TextLink>
      </p>
    </Shell>
  );
}

...
import { applyRules } from "../src/underwrite/rules.js";
...
describe("computeTerms", () => {
  ...
  it("decaying Ratspeak-like profile (large d30, tiny d1) -> small base, strong (min-clamped) decay", () => {
    const r = rev({ revenueMicroUsd: { d1: 1_000n, d7: 700_000n, d30: 90_000_000n } });
    const terms = computeTerms(r, quality(10000), { network: "mainnet" });

    // base = min(r7=100_000, r30=3_000_000, (r1+r7)/2=50_500) = 50_500
    // decayBps = clamp(10000*100_000/3_000_000, 5000, 10000) = clamp(333, 5000, 10000) = 5000 (min-clamped: strong decay)
    expect(terms.projected90dMicroUsd).toBe(1_588_145n);
    expect(terms.capMicroUsd).toBe(790_000n); // ~$0.79 — small, decay-aware cap
    expect(terms.minPrincipal).toBe(316_000n);
    expect(terms.drawLimit).toBe(100_000n); // minPrincipal/14 (22_571) floored up to the 100_000 minimum
  });
  ...
  it("below-minimum: a small but nonzero cap produces minPrincipal under the $1 floor", () => {
    ...
    expect(terms.minPrincipal).toBe(792_000n); // < 1_000_000n ($1) -> caller must deny below_minimum
  ...
  it("floorCents never exceeds the 95-cent maximum", () => {
    const r = rev({
      revenueMicroUsd: { d1: 1_000_000n, d7: 7_000_000n, d30: 30_000_000n },
      ageSeconds: 1n * DAY,
    });
    const terms = computeTerms(r, quality(1000), { network: "mainnet" });
    expect(terms.floorCents).toBeLessThanOrEqual(95);
  });
  ...
  it("an explicit env override wins over the network default (hard ceiling, auctionBlocks, draw/grace periods)", () => {
    ...
    const env: UnderwritingEnv = {
      network: "mainnet",
      hardCeilingMicroUsd: 1_000_000n, // $1 — well under the $45 raw cap
      auctionBlocks: 500n,
...
describe("computeTerms (real fixture: Ratspeak)", () => {
  ...
  it("decay compresses the 90d projection well below a naive 3x-of-30d extrapolation, and the full pipeline approves at the mainnet $25 ceiling", async () => {
    ...
    // Fully deterministic given the committed fixture (recorded 2026-09-17).
    expect(revenue.revenueWei).toEqual({
      d1: 12_147_120_945_590_387n,
      d7: 601_208_604_846_368_750n,
      d30: 14_469_954_423_396_483_388n,
    });
    ...
    expect(revenue.ageSeconds).toBe(10_472_632n); // ~121 days old, well past every age haircut/deny
    ...
    // Ratspeak's Bankr `dailyEarnings` happen to be all-zero for the trailing 7 days at
    // recording time (its revenue is bursty, not daily) -> cv is undefined, not a signal.
    const recentDailyRevenue = bankr.dailyEarnings
      .slice(-7)
      .map((d) => BigInt(Math.round(Number(d.weth) * 1e18)));
    ...
    expect(quality.cv).toBeUndefined();
    ...
    expect(terms.projected90dMicroUsd).toBe(3_723_140_956n);
    expect(terms.projected90dMicroUsd).toBeLessThan(revenue.revenueMicroUsd.d30);

...
/**
 * Per-chain addresses, every one verified live on-chain before being recorded here, not
 * copied from docs: `weth`/`usdc`/`ethUsdFeed` by `symbol()`/`decimals()`/`description()`;
 * `poolManager` read directly off `dopplerFeesManager`'s own `poolManager()` immutable
 * getter; `dopplerAirlock` verified via `getModuleState(dopplerFeesManager) ==
 * PoolInitializer (3)` and a real `getAssetData` lookup against a known token on that chain.
 */
